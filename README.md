...
### Single command to run project (requires you cloned and cd into root directory)
`docker compose down -v && docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d --build` run from root directory of project.

### Linux/Mac (clones, cd's and runs project)
```git clone https://github.com/kwmaharaj/capitec-transaction-dispute.git && cd capitec-transaction-dispute && docker compose down -v && docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d --build```

### Windows  (clones, cd's and runs project)
```git clone https://github.com/kwmaharaj/capitec-transaction-dispute.git ; cd capitec-transaction-dispute ; docker compose down -v ; docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d --build```
