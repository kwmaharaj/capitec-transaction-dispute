...
export default function SupportDisputesPage() {
  ...
  async function load() {
    ...
    setErr(null)
    try {
      const data = await apiFetch<PageResponse<DisputeViewRow>>('/v1/support/disputes/view', { auth, query })
      setPageData(data)
    } catch (e: any) {
      const ae = e as ApiError
      setErr(ae.detail ?? ae.title ?? 'Failed to load support disputes')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginBottom: 12 }}>
        ...
        <button className="btn btn-primary" onClick={() => { setPage(0); void load() }}>Search</button>
          ...
          <tbody>
            {rows.map(r => (
              <tr key={r.disputeId} >
                ...
                <td><Link to={`/support/disputes/${r.disputeId}`}>View</Link></td>
              </tr>
            ))}
          </tbody>
        </table>
